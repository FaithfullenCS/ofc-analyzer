...
**Ключевые отличия от Express-версии:**
- Нет `app.listen()` - Vercel управляет запуском
- Экспорт: `module.exports = async (req, res) => {...}` вместо `module.exports = app`
- CORS headers устанавливаются вручную через `res.setHeader()`
- Маршрутизация через проверку `req.url` вместо `app.post()`
